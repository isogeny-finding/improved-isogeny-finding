...
def small_roots(f, constant_polys, bounds, p, sol, m=1, d=None, method="coron"):
  if not d:
    d = f.degree()
  N = p
  ...
  if f.nvariables() == 1:
    ...
    return coron(f, bounds[0], bounds[1])
  elif f.nvariables() == 3:
    ...
    if method == "coron":
      return coron_trivariate(f, bounds[0], bounds[1], bounds[2], l=0)
    ...
    return coron_quadvariate(f, bounds[0], bounds[1], bounds[2], bounds[3], l=0)